...
CANON = (
    1193
)  # decimal from hex value from lsusb or http://www.pcidatabase.com/vendors.php
BRAND = CANON
...
def detect_cams():
    ...
    if CAMS == 2:
        GPHOTOCAM1 = cmdoutput(
            "gphoto2 --auto-detect|grep usb|sed -e 's/.*Camera *//g'|head -n1"
        )
        GPHOTOCAM2 = cmdoutput(
            "gphoto2 --auto-detect|grep usb|sed -e 's/.*Camera *//g'|tail -n1"
        )
        print(GPHOTOCAM1, "is gphotocam1")
        print(GPHOTOCAM2, "is gphotocam2")

        GPHOTOCAM1ORIENTATION = cmdoutput(
            "gphoto2 --port "
            + GPHOTOCAM1
            + " --get-config /main/settings/ownername|grep Current|sed -e's/.*\ //'"
        )
        GPHOTOCAM2ORIENTATION = cmdoutput(
            "gphoto2 --port "
            + GPHOTOCAM2
            + " --get-config /main/settings/ownername|grep Current|sed -e's/.*\ //'"
        )
    ...
    if GPHOTOCAM1ORIENTATION == "left":
        ...
        print(
            GPHOTOCAM1,
            "owner name is neither set to left or right. Please configure that before continuing.",
        )
        restart_program()
...
def set_zoom():
    ...
    for cam in LEFTCAM, RIGHTCAM:
        ...
        cmd = (
            ...
            + " --dev="
            + cam
            + " --chdk='lua while(get_zoom()<3) do click(\"zoom_in\") end'"
        )
        p = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
        sleep(PAUSE)
        cmd = (
            ...
            + " --dev="
            + cam
            + " --chdk='lua while(get_zoom()>3) do click(\"zoom_out\") end'"
        )
        p = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
        sleep(PAUSE)
    sleep(PAUSE)
...
def flash_off():
    ...
    for cam in LEFTCAM, RIGHTCAM:
        cmd = (
            ...
            + " --dev="
            + cam
            + " --chdk='lua while(get_flash_mode()<2) do click(\"right\") end'"
        )
        p = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
        sleep(SHORTPAUSE)
...
def download_from_cams():
    ...
    for side in "left", "right":
        ...
        chmod(SCANDIRPREFIX + TIMESTAMP + "/" + side, perms)
    chown(SCANDIRPREFIX + TIMESTAMP, 1000, 1000)
    chmod(SCANDIRPREFIX + TIMESTAMP, perms)
    ...
    for root, dirs, files in walk(SCANDIRPREFIX + TIMESTAMP):
        for filename in files:
            ...
            chmod(thisfile, stat.S_IRWXU | stat.S_IRGRP | stat.S_IROTH)
            counter += 1
...
def set_ndfilter():
    ...
    for cam in LEFTCAM, RIGHTCAM:
        print(
            "Disabling neutral density filter for",
            cam,
